...
- **Reverse proxy:** Traefik v3 — natywny Docker provider, autodiscovery przez labele, zero konfiguracji per-projekt
...
- **Dynamiczne porty:** port-router.js (Node.js HTTP/WS proxy) w kontenerze, Traefik kieruje `{name}-{port}.localhost` do niego
...
- [x] **T2.1** infra.yml z Traefik: sieć `external`, autodiscovery kontenerów przez Docker labels
